...
from phishing_pandas_analytics import (
    ANALYSIS_REGISTRY,
...
from data_manager import get_primary_cache, refresh_primary_cache, get_profile_dataframe as dm_get_profile_dataframe, get_cache_stats, clear_primary_cache
...
# FIX: Lowered default threshold to 0.45 to better fit the actual probability distribution
HIGH_RISK_THRESHOLD = float(os.getenv("HIGH_RISK_THRESHOLD", "0.45"))
...
def load_feature_columns():
    ...
    _, ext = os.path.splitext(FEATURE_COLUMNS_PATH.lower())
...
def build_campaign_search_filters(campaign_month=None, campaign_year=None, campaign_name=None):
    ...
    if name:
        return {"search_type": "campaign_name", "campaign_pattern": name}
    if month and year:
        return {"search_type": "month_year", "month_pattern": month, "year_pattern": year}
    if month:
        return {"search_type": "month_only", "month_pattern": month}
    if year:
        return {"search_type": "year_only", "year_pattern": year}
...
def build_ui_summary(tool_name, result):
    ...
    if tool_name == "run_analytics":
        ...
        highest = res_data.get("highest_risk_group") or {}
        if highest:
...
def recommend_actions(mode: str = "employee_improvement", brid: str = None, group_by: list = None, filters: dict = None, top_n: int = 5, user_role: str = "user"):
    ...
    try:
        ...
        if mode in ["employee_improvement", "employee", "user", "brid"]:
            ...
            profile_df = get_profile_dataframe()
            profile = get_employee_improvement_profile(profile_df, brid, user_role=user_role)
            if not profile:
            ...
            recommendations = profile.get("recommended_actions", []) or profile.get("improvement_guidance", []) or []
            training_recommendations = profile.get("training_recommendations", []) or []
            return clean_json_value({
                "status": "success",
                "mode": mode,
        ...
        if mode in ["group_recommendations", "group", "department", "city"]:
            analysis_type = "overall_analysis" if not group_by else ("department_performance" if "department" in str(group_by).lower() else "city_performance")
            result = run_analytics(analysis_type=analysis_type, group_by=group_by, filters=filters, top_n=top_n, user_role=user_role)
            if result.get("status") != "success":
                return result
                
            res_data = result.get("result") or result
            actions = res_data.get("recommended_actions", []) or res_data.get("recommendations_actions", []) or []
            return clean_json_value({
                "status": "success",
                "mode": mode,
                "analysis_type": analysis_type,
                ...
                "highest_risk_group": res_data.get("highest_risk_group", {}),
                "highest_reporting_group": res_data.get("highest_reporting_group", {}),
                "highest_no_action_rules": res_data.get("highest_no_action_rules", {}),
                "top_highest_risk_groups": res_data.get("top_highest_risk_groups", []),
                "recommended_actions": actions,
                "recommendations_actions": actions,
                "training_recommendations": res_data.get("training_recommendations", []),
                "risk_analysis": res_data.get("risk_analysis", {}),
                "llm_context": improvement_llm_context(),
            })

        if mode in ["overall_recommendations", "overall", "general"]:
            result = run_analytics(analysis_type="overall_analysis", group_by=[], filters=filters, top_n=5, user_role=user_role)
            if result.get("status") != "success":
                return result
...
def simulation_users(campaign_month: str = None, campaign_year: str = None, campaign_name: str = None, event_type: str = "clicked link", user_role: str = "user", limit: int = 5000):
    ...
    try:
        ...
        if search_type == "campaign_name":
            pattern = str(search.get("campaign_pattern", "")).lower()
        ...
        if is_admin(user_role):
            ...
            # FIX: Explicitly strip all BRID columns to enforce privacy in user mode
            output_cols = [c for c in output_cols if c not in brid_cols]
            
        # Deduplicate output columns to prevent runtime errors (where multiple same-named columns convert to dataframe objects)
        ...
        for col in cleaned.columns:
            # Fallback type check to guarantee safety against accidental DataFrame mappings
...
def system_info(mode: str = "health"):
    ...
    try:
        ...
        if mode in ["health", "health_check", "status"]:
            ...
            db_connected = False
            model_available = bool(PREDICTION_MODEL_PATH and os.path.exists(PREDICTION_MODEL_PATH))
...
def run_server_tests_core():
    ...
    analytics_test_cases = [
        ...
        {"title": "CAMPAIGN PERFORMANCE", "analysis_type": "campaign_performance", "group_by": ["campaignname"], "filters": {}},
        {"title": "TEMPLATE PERFORMANCE", "analysis_type": "template_performance", "group_by": ["templatename"], "filters": {}},
        {"title": "SUBJECT PERFORMANCE", "analysis_type": "subject_performance", "group_by": ["templatesubject"], "filters": {}},
