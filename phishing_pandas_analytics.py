...
import numpy as np
import pandas as pd
from Config import BRID_COLUMN, EMPLOYEE_ID_COLUMNS, PERSONAL_COLUMNS, apply_feature_engineering, target_function
...
def ensure_target_column(df):
    try:
        ...
        if "eventtype" in df.columns:
            normalized_eventtype = df["eventtype"].fillna("").astype(str).str.strip().str.lower().str.replace(r"\s+", " ", regex=True)
            df["target"] = normalized_eventtype.map(target_function).fillna(-1).astype(int)
        else:
            df["target"] = -1
        return df
    except Exception:
...
def add_time_features(df):
    try:
        df = df.copy()
        sent_col = resolve_column(df.columns, ["senttimestamp", "senttime", "sent_time", "timestamp"])
...
def _to_record_list(value):
    try:
        ...
        if isinstance(value, pd.DataFrame):
            return clean_json_value(value.to_dict("records"))
        if isinstance(value, pd.Series):
            return clean_json_value(value.to_dict())
        if isinstance(value, list):
...
def resolve_dimension_column(column_by_groups, columns):
    mapping = {
        ...
        # FIX: Ensure "sent_month" (YYYY-MM format) resolves before raw integer numbers
        "month": ["sent_month", "sent_month_num", "sent_month_name", "month"],
        "month_year": ["sent_month", "month_year"],
        ...
        "employee": ["usertags-BRID", "proofpoint_brid", "brid", "BRID", "employee", "employee_id", "user", "user_id"],
...
def apply_filters(df, filters):
    try:
        ...
        for column, value in filters.items():
            ...
            if column_name in ["month", "campaign_month"]:
                month_value = str(value or "").strip().lower()
                month_code = normalize_month_value(month_value)
                month_candidates = ["sent_month_num", "sent_month_name", "sent_month"]
                matched = False
                for candidate in month_candidates:
                    if candidate in res_df.columns:
                        if candidate == "sent_month_num":
                            numeric_values = pd.to_numeric(res_df[candidate], errors="coerce")
                            filtered_df = res_df[numeric_values.notna() & (numeric_values == int(month_code))]
                        elif candidate == "sent_month_name":
                            month_name_variants = {month_value, month_value[:3], month_code, month_code.zfill(2)}
                            filtered_df = res_df[res_df[candidate].fillna("").astype(str).str.lower().isin(month_name_variants)]
                        else:
                            filtered_df = res_df[res_df[candidate].fillna("").astype(str).str.lower().str.contains(month_value, regex=False)]
                ...
                continue
            elif column_name in ["year", "campaign_year"]:
                ...
                if "sent_year" in res_df.columns:
                    ...
                    if not filtered_df.empty:
                        res_df = filtered_df
                        continue
            elif column_name in ["month_year", "campaign_month_year"]:
                month_year_value = str(value or "").strip().lower()
                month_code = None
                year_value = None

                tokens = re.split(r"[\s\-/]+", month_year_value)
                for token in tokens:
                    if token.isdigit() and len(token) == 4:
                        year_value = token
                    elif token:
                        parsed_month = normalize_month_value(token)
                        if parsed_month and parsed_month.isdigit() and len(parsed_month) == 2:
                            month_code = parsed_month

                if month_code and year_value:
                    filtered_df = pd.DataFrame(columns=res_df.columns)
                    if "sent_month" in res_df.columns:
                        filtered_df = res_df[res_df["sent_month"].fillna("").astype(str).str.lower().str.startswith(f"{year_value}-{month_code}")]
                    elif "sent_year" in res_df.columns and "sent_month_num" in res_df.columns:
                        filtered_df = res_df[(res_df["sent_year"].fillna("").astype(str) == year_value) & (pd.to_numeric(res_df["sent_month_num"], errors="coerce") == int(month_code))]
...
def build_recommended_actions(summary_df):
    try:
        ...
        metric_cols = {"total_events", "clicked_count", "reported_count", "no_action_count", "result", "click_rate_percent", "report_rate_percent", "no_action_rate_percent", "risk_score"}
        
        # FIX: Find the first actual dimension grouping column 
        dim_col = None
        ...
        group_label = str(highest_risk_group.get(dim_col, "Medium")) if dim_col else "Medium"
        is_emp_col = is_employee_id_column(dim_col) if dim_col else False
        ...
        if float(highest_risk_group.get("click_rate_percent", 0) or 0) >= 20:
            ...
            # Add a meta-flag to trigger downstream privacy masking if this is an employee ID
            if is_emp_col:
                action_obj["_is_employee_target"] = True
...
def build_metric_summary(df, dimensions):
    try:
        ...
        # FIX: Handle empty dimensions for overall aggregate summaries
        if not dimensions:
            res_df["_overall_group"] = "Overall Organisation"
            resolved_cols = ["_overall_group"]
        else:
            for dim in dimensions:
                col = resolve_dimension_column(dim, res_df.columns)
        ...
        result = res_df.groupby(resolved_cols, dropna=False).agg(
...
def get_employee_profile(profile_cache_df, brid, user_role="user"):
    try:
        ...
        return sanitize_record_for_role(profile, user_role=user_role, allow_employee_id=False)
    except Exception:
        log_error(traceback.format_exc())
        return {}
...
def get_top_risky_employees(profile_cache_df, limit=10, user_role="user"):
    try:
        ...
        return sanitize_records_for_role(result, user_role=user_role, allow_employee_id=False)
    except Exception:
        log_error(traceback.format_exc())
        return []
...
def get_employee_improvement_profile(profile_cache_df, brid, user_role="user"):
    try:
        ...
        output = sanitize_record_for_role(output, user_role=user_role, allow_employee_id=False)
...
class GenericAnalyzer(BaseAnalyzer):
    def run(self, df, dimensions, top_n, analysis_type=None):
        ...
        return {
            ...
            "top_clicked_subjects": _to_record_list(get_top_value_counts_by_candidates(clicked_df, ["templatesubject", "subject"], top_n)),
            "top_clicked_templates": _to_record_list(get_top_value_counts_by_candidates(clicked_df, ["templatename", "template"], top_n)),
            "top_clicked_campaigns": _to_record_list(get_top_value_counts_by_candidates(clicked_df, ["campaignname", "campaign"], top_n)),
...
def sanitize_result_for_role(result, user_role="user", allow_employee_id=False):
    try:
        # FIX: Admin specific cleanup to strip internal flags 
        if is_admin(user_role):
            if result and "recommended_actions" in result and isinstance(result.get("recommended_actions"), list):
                for action in result["recommended_actions"]:
                    action.pop("_is_employee_target", None)
            return result
        ...
        if "summary" in result and isinstance(result["summary"], list):
            result["summary"] = sanitize_records_for_role(result["summary"], user_role=user_role, allow_employee_id=allow_employee_id)

        for key in ["highest_risk_group", "highest_reporting_group", "lowest_risk_groups", "lowest_risk_group", "highest_waste_group"]:
            if key in result and isinstance(result[key], list):
                result[key] = sanitize_records_for_role(result[key], user_role=user_role, allow_employee_id=allow_employee_id)
            elif key in result and isinstance(result[key], dict):
                result[key] = sanitize_record_for_role(result[key], user_role=user_role, allow_employee_id=allow_employee_id)
        ...
        if "insights" in result and isinstance(result["insights"], list):
            ...
            for item in result["insights"]:
                ...
                if isinstance(item.get("value"), dict):
                    item["value"] = sanitize_record_for_role(item["value"], user_role=user_role, allow_employee_id=allow_employee_id)
                sanitized_insights.append(item)
            result["insights"] = sanitized_insights
            
        # FIX: Check the dynamically populated target metadata block in recommendations
        if "recommended_actions" in result and isinstance(result["recommended_actions"], list):
            sanitized_actions = []
            for action in result["recommended_actions"]:
                action_copy = dict(action)
                # Safely retrieve and remove the metadata flag
                is_emp_target = action_copy.pop("_is_employee_target", False)
                if is_emp_target and not allow_employee_id:
                    action_copy["target"] = "<BRID>"
                sanitized_actions.append(action_copy)
            result["recommended_actions"] = sanitized_actions
...
def run_analysis(df, analysis_type, group_by=None, filters=None, user_role="user", top_n=DEFAULT_TOP_N, metadata=None):
    try:
        ...
        output = {
            ...
            "result": sanitize_result_for_role(result, user_role=user_role, allow_employee_id=allow_id)
        }
