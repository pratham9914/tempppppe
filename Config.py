...
def _combine_text_columns(*cols):
    ...
    for col in cols:
        if _is_series(col):
            max_len = len(col)
            break
    output = pd.Series([''] * max_len, dtype="string")
    for col in cols:
        if _is_series(col):
            current = _safe_text_series(col).reset_index(drop=True)
        else:
            current = pd.Series([_safe_text_scalar(col)] * max_len, dtype="string")
        output = output.astype("string") + " " + current.astype("string")
    return _safe_text_series(output.astype(str))
...
def _save_model(model, path=PREDICTION_MODEL_PATH):
    if not path:
        raise ValueError("No path provided for saving model. Set PREDICTION_MODEL_PATH or provide an explicit path.")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'wb') as f:
        pickle.dump(model, f)

def _load_model(path=PREDICTION_MODEL_PATH):
    if not path:
        raise ValueError("No path provided for loading model. Set PREDICTION_MODEL_PATH or provide an explicit path.")
